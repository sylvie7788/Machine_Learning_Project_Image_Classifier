...
class CNNSentimentKim(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        # layer1 = self.conv1.forward(embeddings).relu()
        # layer2 = self.conv2.forward(embeddings).relu()
        # layer3 = self.conv3.forward(embeddings).relu()
        # s = minitorch.nn.max(layer1, 2) +  minitorch.nn.max(layer2, 2) +  minitorch.nn.max(layer3, 2)
        # l = self.linear.forward(s)
        ...
        layer2 = self.conv2.forward(embeddings).relu()
        layer3 = self.conv3.forward(embeddings).relu()
        out_sum = minitorch.nn.max(layer1, 2) + minitorch.nn.max(layer2, 2) + minitorch.nn.max(layer3, 2)
        l = self.linear.forward(out_sum)  #  Linear forward : batch, in_size, _ = x.shape
        if self.training:
            l = minitorch.dropout(l, self.dropout)
        return l.sigmoid().view(embeddings.shape[0])
