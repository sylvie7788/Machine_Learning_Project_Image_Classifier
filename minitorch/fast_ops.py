...
def tensor_zip(
    ...
    * When `out`, `a`, `b` are stride-aligned, avoid indexing
    ...
    def _zip(
        ...
        # TODO: Implement for Task 3.1.
        if (
            (len(out_strides != len(a_strides)) or (len(out_strides != len(b_strides))))
...
def tensor_reduce(
    ...
    * Inner-loop should not call any functions or write non-local variables
    ...
    def _reduce(
        ...
        for i in prange(len(out)):
            out_index: Index = np.zeros(MAX_DIMS, np.int32)
            reduce_size = a_shape[reduce_dim]  # to iterate through
            to_index(i, out_shape, out_index)
            o = index_to_position(out_index, out_strides)
            accm = out[o]
            j = index_to_position(out_index, a_strides)  # map
            step = a_strides[reduce_dim]  # global read
            for s in range(reduce_size):
                accm = fn(accm, a_storage[j])
                j += step
            out[o] = accm
        # raise NotImplementedError("Need to implement for Task 3.1")
    ...
    for i in prange(out_shape[0]):
        for j in prange(out_shape[1]):
            for k in prange(out_shape[2]):
                a = i * a_batch_stride + j * a_strides[1]
                b = i * b_batch_stride + k * b_strides[2]
                acc = 0.0
                for h in range(a_shape[2]):
                    acc += a_storage[a] * b_storage[b]
                    a += a_strides[2]
                    b += b_strides[1]
                final_position = (
                    i * out_strides[0] + j * out_strides[1] + k * out_strides[2]
                )
                out[final_position] = acc
