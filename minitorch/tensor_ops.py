...
def tensor_map(fn: Callable[[float], float]) -> Any:
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _map(
        ...
        big_index = np.zeros(MAX_DIMS, np.int32)
        ...
        for i in range(len(out)):
            ...
            out[index_to_position(big_index, out_strides)] = fn(
...
def tensor_zip(fn: Callable[[float, float], float]) -> Any:
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _zip(
        ...
        for i in range(len(out)):
            to_index(i, out_shape, big_index)
            broadcast_index(big_index, out_shape, a_shape, a_index)
            broadcast_index(big_index, out_shape, b_shape, b_index)
            ind = index_to_position(big_index, out_strides)
            a_value = a_storage[index_to_position(a_index, a_strides)]
            b_value = b_storage[index_to_position(b_index, b_strides)]
            out[ind] = fn(a_value, b_value)
    ...
    * `out_shape` will be the same as `a_shape`
    ...
    def _reduce(
        ...
        for i in range(len(out)):
            ...
            for o in range(reduce_size):
                out_index[reduce_dim] = o
                j = index_to_position(out_index, a_strides)
                out[k] = fn(out[k], a_storage[j])
